...
void aggregate_model(const VectorXd& local_update) {
    ...
    if (!local_update.allFinite()) {
        std::cerr << "[ERROR] Received weights contain NaN or inf values. Ignoring update." << std::endl;
        return;
    }

    // Initialize global_weights and total_weights with the correct size if not already done
    ...
    global_weights = (1 - scaling_factor) * global_weights + scaling_factor * local_update;

    client_count++;

    if (!global_weights.allFinite()) {
        std::cerr << "[ERROR] Global weights contain NaN or inf values. Resetting weights." << std::endl;
