...
void generate_dummy_data(std::vector<std::vector<double>>& features, std::vector<double>& labels, 
                        int n_samples = 10000, int n_features = 20) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::normal_distribution<> feature_dist(0.0, 1.0);
    std::normal_distribution<> noise_dist(0.0, 0.1);
    
    // Generate true weights for the underlying linear relationship
    ...
    for (int i = 0; i < n_samples; ++i) {
        ...
        for (int j = 0; j < n_features; ++j) {
            ...
            target += true_weights[j] * features[i][j];
        }
        
        // Add noise to target
        labels[i] = target + noise_dist(gen);
    }
    
    std::cout << "[INFO] Generated " << n_samples << " samples with " << n_features << " features" << std::endl;
}
...
int main() {
    try {
        ...
        // Generate dummy data instead of loading from CSV
        generate_dummy_data(features, labels, 10000, 20);  // 10k samples, 20 features
